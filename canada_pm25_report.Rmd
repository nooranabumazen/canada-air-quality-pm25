...
```{css}
.narrative { font-size:0.95rem; color:#8a7e6f; line-height:1.7; padding:0 1rem; }
...
@media (max-width: 768px) {
  .insight-box { margin-bottom: 0 !important; }
  [data-height] { min-height: 0 !important; height: auto !important; }
  .chart-shim { overflow: visible !important; min-height: 0 !important; }
}
```
...
plot_ly(city_summary) %>%
  ...
  layout(legend = list(y = -0.35), margin = list(b = 50)) %>%
  layout(xaxis=list(range=c(0,18), standoff = 5),yaxis=list(showgrid=FALSE),
    shapes=list(
